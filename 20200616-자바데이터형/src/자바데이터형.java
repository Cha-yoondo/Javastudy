 ...
 *   컴퓨터는 0,1 => bit
 * 정수
 *  = byte (0,1을 8개를 저장하는 공간) => 8 bit(1byte) ==> web 주로 사용
 *    11111111 ==> -128
 *    = 음수
 ...
 *    =========> C언어와 호환성 (사용빈도x)
 *  = int (평상시에 사용하는 모든 정수는 컴퓨터에서 int로 인식) => dafault
 *    (0,1 => 32개를 저장하는 공간)
 ...
 * 문자
 *     = char(0,1 => 16 bit) : 음수는 없이 양수만 ==> 0~65535
 *     * 문자마다 고유의 번호를 갖고있다
 *     'A' ==> 65
 *     'a' ==> 97
 *     '0' ==> 49
 ...
 *               a != A
 *               
 *   2) 숫자 사용이 가능 ( 단 앞에는 사용할수없다)
 *      a1,a2......(2a(error))
 *      
 *   3) 특수문자 삳용이 가능 ( _ ,$)
 *      _ => 임시파일,단어가 여러개일경우에 주로 사용
 *           file_size, file_name
 *           
 *   4) 키워드 (자바에서 사용중인 단어)는 사용할수없다
 *      int int1
 *      
