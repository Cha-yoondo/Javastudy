...
//	    System.out.println("dao 실행 count: "+count);
   ...
   {
	   request.setAttribute("main_jsp", "../member/login_main.jsp");//include(메인에 들어갈 파일 지정)
	   return "../main/main.jsp";//include
   }
   ...
   public String member_login(HttpServletRequest request)
   {
	   ...
	   // DAO로 전송 결과값을 가지고 온다 
	   // 받은 결과값을 ==> login.jsp전송 
	   MemberVO vo=MemberDAO.memberLogin(member_id, pwd);
	   if(vo.getMsg().equals("OK"))
	   {
		   ...
		   session.setAttribute("admin_type", vo.getAdmin_type());
